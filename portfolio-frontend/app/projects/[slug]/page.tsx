...
import { useLanguage } from "@/app/i18n/LanguageContext";
import Link from "next/link";
import { useTranslationStyle } from "@/app/context/TextStyleContext";
...
export default function ProjectDetailDisplay() {
  ...
  // 🆕 Chỉ mapping key "latest"
  const styleLatest = useTranslationStyle("latest");
      ...
      {!latestLoading && latestProjects.length > 0 && (
        <div className="mt-12 border-t pt-8 md:pt-12">
          <div className="flex items-center justify-between mb-6 gap-4">
            {/* 🆕 Chỉ mapping key "latest" */}
            <h2
              className="text-[36px] sm:text-[48px] md:text-[64px] font-medium tracking-tight leading-none"
              style={styleLatest}
            >
              {t("latest")}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {latestProjects.map((project) => (
              ...
              >
                ...
                />
                <span
                  className="absolute bottom-2 left-3 text-white text-[18px] sm:text-[20px] md:text-[24px] mix-blend-difference"
