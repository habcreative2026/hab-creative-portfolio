...
import "@/app/admin/globals.css";
import toast from "react-hot-toast";
import CardStyleEditor, {
  type CardStyleData,
} from "./components/CardStyleEditor";
...
const DEFAULT_STYLE: CardStyleData = {
  font: "",
  weight: 0,
  size: 0,
  color: "",
...
export default function CardProjectAdmin() {
  ...
  // 🆕 Style states
  const [titleStyle, setTitleStyle] = useState<CardStyleData>(DEFAULT_STYLE);
  const [clientStyle, setClientStyle] = useState<CardStyleData>(DEFAULT_STYLE);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("clientStyle", JSON.stringify(clientStyle));
  ...
  const handleEditClick = async (card: CardProject) => {
    ...
    // 🆕 Load styles
    setTitleStyle({
      font: card.titleStyle?.font || "",
      weight: card.titleStyle?.weight || 0,
      size: card.titleStyle?.size || 0,
      color: card.titleStyle?.color || "",
    });

    setClientStyle({
      font: card.clientStyle?.font || "",
      weight: card.clientStyle?.weight || 0,
      size: card.clientStyle?.size || 0,
      color: card.clientStyle?.color || "",
    });
  ...
  const resetForm = () => {
    ...
    setProjectsPageImageUrl("");
    // 🆕 Reset styles
    setTitleStyle(DEFAULT_STYLE);
    setClientStyle(DEFAULT_STYLE);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="scroll-none lg:col-span-5 lg:max-h-[calc(90vh)] lg:overflow-y-auto bg-[#0F172A] p-4 sm:p-6 rounded-2xl border border-[#1E293B] space-y-4 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* 🆕 Client Style */}
              <CardStyleEditor
                label="Style cho Tập đoàn"
                value={clientStyle}
                onChange={setClientStyle}
                accentColor="emerald"
              />
