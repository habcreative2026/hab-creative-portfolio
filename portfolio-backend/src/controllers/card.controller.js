...
function parseStyle(raw) {
  // Nếu không gửi → undefined (skip update)
  ...
  // Luôn trả về object đầy đủ (kể cả rỗng)
  return {
    font: obj?.font || "",
...
exports.createCard = async (req, res) => {
  try {
    const {
      ...
      homeImageUrl,
      projectsPageImageUrl,
      // 🆕 Style raw (JSON string từ FormData)
      titleStyle: titleStyleRaw,
      clientStyle: clientStyleRaw,
    } = req.body;
    ...
    // 🆕 Parse styles
    const titleStyle = parseStyle(titleStyleRaw);
    const clientStyle = parseStyle(clientStyleRaw);
...
exports.updateCard = async (req, res) => {
  try {
    ...
    // ✅ Update khi được gửi (kể cả rỗng để reset)
    if (titleStyle !== undefined) {
      updateData.titleStyle = titleStyle;
    }
    if (clientStyle !== undefined) {
